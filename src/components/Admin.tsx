...
import { LayoutDashboard, Users, FileCheck, DollarSign, MessageSquare, Search, MoreVertical, ShieldAlert, CheckCircle, XCircle } from 'lucide-react';
    ...
    <div className="flex h-screen overflow-hidden bg-[#0B0B0B]">
      ...
      <div className={`glass-panel border-y-0 border-l-0 rounded-none transition-all duration-300 flex flex-col ${sidebarOpen ? 'w-64' : 'w-20'}`}>
        <div className="p-6 flex items-center gap-3 h-[72px] border-b border-white/10">
          <div className="w-8 h-8 bg-[#FFD600] rounded-lg flex items-center justify-center text-black font-bold shrink-0">A</div>
        ...
        <div className="flex-1 py-6 px-3 flex flex-col gap-2">
          {navItems.map(item => (
            <button 
              ...
              className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${screen === item.id ? 'bg-[#FFD600]/10 text-[#FFD600]' : 'text-white/70 hover:bg-white/5 hover:text-white'}`}
            >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="h-[72px] glass-panel border-x-0 border-t-0 rounded-none flex items-center justify-between px-6 z-10">
          ...
          <div className="flex items-center gap-4">
            <div className="text-sm text-white/70">Admin User</div>
  ...
  <div className="max-w-6xl mx-auto">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <GlassCard className="h-80 flex flex-col">
        ...
        <div className="flex-1 bg-[linear-gradient(to_right,#ffffff0a_1px,transparent_1px),linear-gradient(to_bottom,#ffffff0a_1px,transparent_1px)] bg-[size:20px_20px] rounded-xl relative overflow-hidden">
          <div className="absolute top-[20%] left-[30%] w-32 h-32 bg-red-500/40 blur-2xl rounded-full" />
          <div className="absolute top-[50%] left-[60%] w-40 h-40 bg-[#FFD600]/40 blur-2xl rounded-full" />
  ...
  <div className="max-w-6xl mx-auto">
    <div className="flex justify-between items-center mb-6">
      ...
      <div className="flex bg-black/40 p-1 rounded-xl">
        <button className="px-6 py-2 rounded-lg bg-[#FFD600] text-black font-medium text-sm">Passengers</button>
    ...
    <GlassCard className="p-0 overflow-hidden">
      <div className="p-4 border-b border-white/10 flex gap-4">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search users..." className="w-full bg-black/20 border border-white/10 rounded-lg py-2 pl-10 pr-4 text-sm focus:border-[#FFD600] focus:outline-none" />
        </div>
        ...
        <GlassCard key={i} className="flex flex-col">
          ...
          <div className="space-y-2 mb-6 flex-1">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-[#FFD600] cursor-pointer hover:underline">View Doc</span>
            </div>
            <div className="flex justify-between text-sm">
  ...
  <div className="max-w-6xl mx-auto">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
      <GlassCard className="lg:col-span-2 flex flex-col justify-center items-center py-12">
        ...
        <div className="text-5xl font-bold text-[#FFD600] mb-4">$142,500</div>
  ...
  <div className="max-w-6xl mx-auto h-[calc(100vh-120px)] flex gap-6">
    <GlassCard className="w-1/3 flex flex-col p-0 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        {[1,2,3,4,5].map(i => (
          <div key={i} className={`p-4 border-b border-white/5 cursor-pointer hover:bg-white/5 ${i === 1 ? 'bg-white/5 border-l-2 border-l-[#FFD600]' : ''}`}>
    ...
    <GlassCard className="flex-1 flex flex-col p-0 overflow-hidden">
      ...
      <div className="flex-1 p-4 overflow-y-auto flex flex-col gap-4">
        ...
        <div className="bg-[#FFD600]/20 text-[#FFD600] p-3 rounded-xl rounded-tr-none max-w-[80%] self-end text-sm">
          We apologize for the experience. We are investigating this immediately.
        </div>
      </div>
      <div className="p-4 border-t border-white/10 flex gap-2">
        <input type="text" placeholder="Type a reply..." className="flex-1 bg-black/20 border border-white/10 rounded-lg px-4 py-2 text-sm focus:border-[#FFD600] focus:outline-none" />
